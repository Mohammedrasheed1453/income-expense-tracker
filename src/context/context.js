...
// 👇 function to properly get initial state
...
export const Provider = ({ children }) => {
    const [transactions, dispatch] = useReducer(contextReducer, [], getInitialTransactions);
    ...
    const balance = transactions.reduce((acc, currVal) => {
        ...
        const amount = isNaN(currVal.amount) ? 0 : currVal.amount;

        // Correct the balance based on transaction type
        return (currVal.type === 'Expense' ? acc - amount : acc + amount);
    }, 0);
